...
export default class DrawCanvas extends Component<props, state> {
  ...
  async componentDidMount () {
    ...
    try {
      ...
      // 总计高度
      let totalHeight = 0
      // 高度计算,先计算高度在绘制，因为先绘制，在计算高度，有时会出问题，原因不详
      // 封面图
      const coverSize = width
      const coverObj: keyValue = await this.getCoverImgSize(info.goodsImage, coverSize, coverSize)
      totalHeight += (coverSize + padding)
      // 价格
      const priceHeight = totalHeight
      totalHeight += this.rate(74)
      // 标题
      let titleHeight = totalHeight
      ctx.setFontSize(this.rate(28))
      totalHeight += this.rate(16)
      totalHeight += this.textRender(ctx, info.activityName, width, padding, totalHeight, this.rate(28), this.rate(12), false)

      // 描述
      let descHeight = totalHeight
      if (info.goodsSoldPoint) {
        ctx.setFontSize(this.rate(22))
        totalHeight += this.rate(17)
        totalHeight += this.textRender(ctx, info.goodsSoldPoint, width, padding, totalHeight, this.rate(22), this.rate(10), false)
      }
      console.log('descHeight', totalHeight)
      // 分割线
      let splitLineHeight = totalHeight
      ...
      ctx.fillText('¥', padding, priceHeight + this.rate(32) + font1)
      const font2 = this.rate(46)
      ctx.setFontSize(font2)
      ctx.fillText(info.moneyFixed, padding + font1, priceHeight + this.rate(22) + font2)
      ...
      if (info.lineMoney) {
        ...
        const font3Top = priceHeight + this.rate(37) + font3
      ...
      ctx.setFontSize(this.rate(28))
      titleHeight += this.rate(16)
      titleHeight += this.textRender(ctx, info.activityName, width, padding, titleHeight, this.rate(28), this.rate(12))
      ...
      if (info.goodsSoldPoint) {
        ctx.setFillStyle('#777A80')
        ctx.setFontSize(this.rate(22))
        descHeight += this.rate(17)
        descHeight += this.textRender(ctx, info.goodsSoldPoint, width, padding, descHeight, this.rate(22), this.rate(10))
      }

      // 画分割线
      ctx.setStrokeStyle('#B07D2E')
      ctx.setLineWidth(this.rate(2))
      splitLineHeight += this.rate(28)
      ctx.moveTo(padding, splitLineHeight)
      ctx.lineTo(padding + width, splitLineHeight)
      ctx.stroke()
      splitLineHeight += this.rate(2)

      // // 画二维码
      console.log('画二维码')
      qrcodeHeight += this.rate(12)
      const qrcodeObj: keyValue = await this.getCoverImgSize(qrcode, qrcodeSize, qrcodeSize)
      ctx.drawImage(qrcodeObj.path, qrcodeObj.sx, qrcodeObj.sy, qrcodeObj.sWidth, qrcodeObj.sHeight, padding, qrcodeHeight, qrcodeSize, qrcodeSize)
      ...
      const cY = qrcodeHeight + qrcodeSize / 2
      ...
      // 长按图片
      const loogTagHeigh = qrcodeHeight + this.rate(46) + textFontSize
  ...
  textRender (ctx, text, width, left, top, fontSzie, lineHeight, isDraw = true) {
    let h = 0
